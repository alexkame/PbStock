...
//对于走势均线的问题，目前的处理规则如下：
//
//1.	对于当日走势的请求里的均线数据，客户端不再自己计算处理，直接使用服务器返回均价字段
//2.	对于推送请求里返回的均价，有两种不同处理
//1)	如果当前是指数，直接使用均价字段
//2)	如果非指数，客户端采用金额/量的方式计算
//
//3.	在画均线的时候，如果均价所有数据为零，则不画均价线；
 *
 * 
 * @className ZQTrendLineView.java
 * 
 */

public class ZQTrendLineView extends FrameLayout {

	public static final String TAG = ZQTrendLineView.class.getSimpleName();
...
//	private final static int POSITION_RIGHT = 2;
	...
	public ZQTrendLineView(Context context, boolean bShowRight) {
		super(context);
	...
	private void initView(Context context) {
		...
		if (mbShowRight) {
			LayoutParams lp1 = new LayoutParams(mScreenSize.widthPixels * 7 /10, LayoutParams.MATCH_PARENT);
			layouthorn.addView(layouthorn1, lp1);

			LayoutParams lp2 = new LayoutParams(mScreenSize.widthPixels * 3 / 10, LayoutParams.MATCH_PARENT);
	...
	protected void drawRule(Canvas canvas) {
		...
		if (mPopinfoFlag == true) {
			...
			//if (indexY >= mTlineTopY && indexY <= mTlineBottomY)
			{
				...
				int nPrice = (int) (mOptionData.HQData.getnLastClear() - mPriceOffset * (m_iYPos - mTlineMidY)/mLineSpace);
				...
				{
					r.set(mLineLeft, mTlineTopY, mLineLeft + nWidth, mTlineTopY + nHeight);
				...
				String strzdf = "----";
				strzdf = ViewTools.getZDF((int) (mPriceOffset * (mTlineMidY - m_iYPos)/mLineSpace), mOptionData.HQData.getnLastClear(), mOptionData.HQData.getnLastPrice(), true, true);
				nWidth = (int) mPaint.measureText(strzdf) + 10;
				
				if ((indexY - nHeight/2) < mTlineTopY)
				{
					r.set(mLineRight - nWidth, mTlineTopY, mLineRight, mTlineTopY + nHeight);
				}else
				{
					r.set(mLineRight - nWidth, indexY - nHeight/2, mLineRight, indexY + nHeight/2);
				}
				canvas.drawRoundRect(r, 5, 5, mPaint);
		...
		protected void drawTrendLine(Canvas canvas) {
			...
			ViewTools.DrawPrice_ZQ(canvas, mLineLeft + 2, tempY, mOptionData.HQData.getnLastClear(), 1, mOptionData.HQData.getnLastClear(), mOptionData.PriceDecimal,mOptionData.PriceRate, mPaint, false);
		
			
			mPaint.setColor(ColorConstant.ZQ_DETAIL_PRICE_TOP);
			tempY -= mLineSpace;
			tempY -= mLineSpace;			
			ViewTools.DrawPrice_ZQ(canvas, mLineLeft + 2, tempY, mOptionData.HQData.getnLastClear() + mPriceOffset * 2, 1, mOptionData.HQData.getnLastClear(),mOptionData.PriceDecimal, mOptionData.PriceRate, mPaint, false);
			...
			if (mOptionData.HQData.getnLastClear() < mPriceOffset * 2) {
				...
				ViewTools.DrawPrice_ZQ(canvas, mLineLeft + 2, tempY, mOptionData.HQData.getnLastClear() - mPriceOffset * 2, 1, mOptionData.HQData.getnLastClear(),
			...
			tempY = mTlineMidY;
			ViewTools.DrawZDF_ZQ(canvas, mLineRight - 2, tempY, 0, 1, mOptionData.HQData.getnLastClear(), true, true, mPaint, false);

			mPaint.setColor(ColorConstant.ZQ_DETAIL_PRICE_TOP);
			tempY -= mLineSpace;
			tempY -= mLineSpace;
			ViewTools.DrawZDF_ZQ(canvas, mLineRight - 2, tempY, mPriceOffset * 2, 1, mOptionData.HQData.getnLastClear(), true, true, mPaint, false);
