 ...
 * 
 */
public class ZQKLineView extends FrameLayout {

	public static final String TAG = ZQKLineView.class.getSimpleName();
	//
	...
	public ZQKLineView(Context context, boolean bOption) {
		super(context);
		mContext = context;		
	...
	public void SetCycle(int cycle) {
		...
		{
			mKLineDataArray = mMyApp.getKLineWeekArray();
		}
		else if(mCycle == Global_Define.HISTORY_TYPE_MONTH)
		{
			mKLineDataArray = mMyApp.getKLineMonthArray();
		...
		{
			mKLineDataArray = mMyApp.getKLineMinArray();
		}else
		{
			mKLineDataArray = mMyApp.getKLineDataArray();
		}
...
//				drawBIAS(canvas);
		...
		protected void drawKLine(Canvas canvas) {
			...
			mPaint.setTextAlign(Paint.Align.LEFT);
			Draw_Price=minvalue;
			ViewTools.DrawPrice_ZQ(canvas, mLineLeft+2, mKlineBottomY-mFontH,Draw_Price,1,Draw_Price, mOptionData.PriceDecimal, mOptionData.PriceRate,mPaint,true);
			...
			ViewTools.DrawPrice_ZQ(canvas, mLineLeft+2, (int)(mKlineBottomY-mLineSpace*2.5-mFontH),Draw_Price,1,Draw_Price, mOptionData.PriceDecimal, mOptionData.PriceRate,mPaint,true);
			...
			mPaint.setTextAlign(Paint.Align.LEFT);
			Draw_Price=maxvalue;
			ViewTools.DrawPrice_ZQ(canvas, mLineLeft+2, mStockPanelY+mFontH,Draw_Price,1,Draw_Price, mOptionData.PriceDecimal, mOptionData.PriceRate,mPaint,true);
...
//			ViewTools.DrawPrice(canvas, mLineLeft + 2, mKlineBottomY - mFontH, mMinPrice, 1, mMinPrice, mOptionData.PriceDecimal, mOptionData.PriceRate, mPaint, true);
			...
			ViewTools.DrawPrice(canvas, mLineLeft+2, (int)(mKlineBottomY-mLineSpace*2.5-mFontH),Draw_Price,1,Draw_Price, mOptionData.PriceDecimal, mOptionData.PriceRate,mPaint,true);
			...
			ViewTools.DrawPrice(canvas, mLineLeft+2, mStockPanelY+mFontH,Draw_Price,1,Draw_Price, mOptionData.PriceDecimal, mOptionData.PriceRate,mPaint,true);
		...
		protected void drawRule(Canvas canvas) {
			...
			if (mPopinfoFlag == true) {
				...
				if (indexY > mKlineTopY && indexY < mKlineBottomY)
				{
					...
					int priceOffset = (int) ((mMaxPriceWithAva - mMinPriceWithAva)/(mKlineBottomY - mKlineTopY - 2*mFontH));
