...
public class ZQLSCJActivity extends HdActivity implements OnClickListener {
			...
			case TradeNetConnect.MSG_UPDATE_DATA: {// 交易
				...
				if (nFrameType == 1)// 交易连接，交换密钥
				{
					...
					if (aMsgObject.nErrorCode < 0) {
						...
						Toast.makeText(ZQLSCJActivity.this, "登录失败", 5000)
								.show();
					} else {
						// login success
				...
				else if (nFrameType == Trade_Define.Func_LSCJ) {
					updateTradeData();
				}
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.zq_trade_lscj_frame);
		initData();
		initView();
		initDate();
	}
	...
	private void initView() {
		...
		mQueryBtn = (Button) findViewById(R.id.lscj_query_btn);
		mQueryBtn.setOnClickListener(this);

		mListView = (ListView) findViewById(R.id.trade_order_listview_cj);

		mCJAdapter = new ZqCJAdapter(mListData_cj, this, mHandler, mMyApp, true);
	...
	private void updateTradeData() {
		mMyApp.mTradeData.GetLSCJ(mListData_cj);
