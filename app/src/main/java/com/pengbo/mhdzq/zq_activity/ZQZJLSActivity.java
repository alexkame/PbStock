...
import com.pengbo.mhdzq.adapter.ZqZJAdapter;
...
public class ZQZJLSActivity extends HdActivity implements OnClickListener {

	private MyApp mMyApp;
	private PBSTEP mListData_lszj;// 历史资金流水
			...
			case TradeNetConnect.MSG_UPDATE_DATA: {// 交易
				...
				if (nFrameType == 1)// 交易连接，交换密钥
				{
					...
					if (aMsgObject.nErrorCode < 0) {
						...
						Toast.makeText(ZQZJLSActivity.this, "登录失败", 5000)
								.show();
					} else {
						// login success
				...
				else if (nFrameType == Trade_Define.Func_LSZJLS) {
					updateTradeData();
				}
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.zq_trade_zjls_frame);
		initData();
		initView();
		initDate();
	}
	...
	private void initView() {
		...
		mQueryBtn = (Button) findViewById(R.id.lszj_query_btn);
		mQueryBtn.setOnClickListener(this);

		mListView = (ListView) findViewById(R.id.trade_order_listview_cj);

		mZJAdapter = new ZqZJAdapter(mListData_lszj, this, mHandler, mMyApp);
	...
	private void requestLSZJLS() {
		mMyApp.mTradeNet.setMainHandler(mHandler);
		String param = "";
		param = String.format("%d:%s|%d:%s", STEP_Define.STEP_QSRQ, mStartDate,
				STEP_Define.STEP_ZZRQ, mEndDate);
		mMyApp.mTradeNet.Request_ListQuery(Trade_Define.Func_LSZJLS, param);
	}

	private void updateTradeData() {
		mMyApp.mTradeData.GetLSZJLS(mListData_lszj);
