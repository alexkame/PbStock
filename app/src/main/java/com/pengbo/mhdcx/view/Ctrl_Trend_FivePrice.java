...
public class Ctrl_Trend_FivePrice extends LinearLayout{
	...
	public final static String TAG = Ctrl_Trend_FivePrice.class.getSimpleName();
	...
	public Ctrl_Trend_FivePrice(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
	}
	...
	public void updateCtrls()
	{
		...
		for(int i=0; i< mOptionData.HQData.sellPrice.length ; i++)
		{
			...
			price_temp = ViewTools.getStringByVolume(mOptionData.HQData.sellVolume[i], mOptionData.market, 1, 6, false);
		...
		for(int i=0; i< mOptionData.HQData.buyPrice.length ; i++){
			...
			price_temp = ViewTools.getStringByVolume(mOptionData.HQData.buyVolume[i], mOptionData.market, 1, 6, false);
