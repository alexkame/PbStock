...
import com.pengbo.mhdcx.adapter.HeadSearchOptionListAdapter;
 ...
 * 
 */
public class HeadSearchOptionListActivity extends BaseActivity implements
	...
	private TextView tv_BiaoDiZDF;// 标的涨跌幅
	private TextView tv_lasttime;// 期权到期日
	...
	private HeadSearchOptionListAdapter mListAdapter;
		...
		public void handleMessage(Message msg) {
			...
			case UPDATE_UI:
				...
				if (mListAdapter == null) {
					mListAdapter = new HeadSearchOptionListAdapter(mMyApp,
							HeadSearchOptionListActivity.this, mDatas,
							mTagCodeInfos);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.context = this;
		mStockData = new TagLocalStockData();
		initTitleView();
		initViewBiaoDi();
		initViewLastTime();
		initListView();
	...
	private void initTitleView() {
		...
		hService = new HeadListDataService(HeadSearchOptionListActivity.this);
	...
	private void initListView() {
		...
		if (mListView1 == null) {
			...
			mListAdapter = new HeadSearchOptionListAdapter(mMyApp,
					HeadSearchOptionListActivity.this, mDatas, mTagCodeInfos);
			mListView1.setAdapter(mListAdapter);
			// 条目点击事件
		...
		mPullToRefreshView = (PullToRefreshView) this
				.findViewById(R.id.optionlist_pull_refreshview);
	...
	private void initViewBiaoDi() {
		...
		tv_BiaoDiZDF = (TextView) this
				.findViewById(R.id.head_option_list_activity_tv2);
	...
	public void onClick(View v) {
		...
		case R.id.head_option_list_activity_close:
			...
			HeadSearchOptionListActivity.this.finish();
			break;

		case R.id.head_option_list_activity_layout1:// 点击 标的 条目时 数据要进行筛选
													// 从新调用数据函数
			myTargetDialog = new MyTargetListDialog(
					HeadSearchOptionListActivity.this);

			adapter2 = new MyTargetDialogAdapter(
					HeadSearchOptionListActivity.this, getTargetTagCodeInfo());
		...
		case R.id.head_option_list_activity_layout2:// 点击 日期条目 时 要 传入此时的 日期参数
													// 还要获得 其他参数的形式
			myDialog = new MyListDialog(HeadSearchOptionListActivity.this)
					.builder();
			MyDialogAdapter adapter_date = new MyDialogAdapter(
					HeadSearchOptionListActivity.this, getDatelists());
	...
	DialogcallbackTarget dialogcallback = new DialogcallbackTarget() {
		...
		public void dialogdo(final int index) {
			...
			if (mMyApp.mStockConfigData.getData(mStockData, stockMarket,
					stockCode, false)) {
				tv_BiaoDiZDF.setVisibility(View.VISIBLE);
				...
				tv_BiaoDiZDF.setText(String.format("%s %s", nowPrice, fd));
				tv_BiaoDiZDF.setTextColor(ViewTools.getColorByFieldID(
						mStockData, Global_Define.FIELD_HQ_NOW));
			} else {
				tv_BiaoDiZDF.setVisibility(View.GONE);
			}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		mIntent.setClass(HeadSearchOptionListActivity.this,
				ScreenDetailActivity.class);

		TagCodeInfo stockCodeInfo = new TagCodeInfo(
				tTagLocalStockData.optionData.StockMarket,
				tTagLocalStockData.optionData.StockCode,
				tTagLocalStockData.group, tTagLocalStockData.name);
		TagCodeInfo optionCodeInfo = new TagCodeInfo(
				tTagLocalStockData.HQData.market,
				tTagLocalStockData.HQData.code, tTagLocalStockData.group,
				tTagLocalStockData.name);
	...
	private void queryHQPushInfo() {
		// comment out these code, because this page not need push data now.
		// ArrayList<TagCodeInfo> codelist = new ArrayList<TagCodeInfo>();
		// for(int i = 0; i < mDatas.size(); i++)
		// {
		// codelist.add(mTagCodeInfos.get(mCurrentListPage*AppConstants.LIST_PAGE_ITEMS_NUM
		// + i));
		// }
		//
		...
		// mRequestCode[0] =
		// GlobalNetProgress.HQRequest_MultiCodeInfoPush(mMyApp.mHQPushNet,codelist,0,codelist.size());
	}
