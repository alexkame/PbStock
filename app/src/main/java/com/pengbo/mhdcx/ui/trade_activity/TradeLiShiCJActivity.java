...
import com.pengbo.mhdcx.adapter.TradeLSCJListAdapter;
...
public class TradeLiShiCJActivity extends HdActivity implements OnClickListener{

	private TextView mTV_Header, mTV_Back;
	private EditText mEdit_StartDate, mEdit_EndDate;
	private Button mBtn_ChaXun;
	private LinearLayout mlLayout_lscjHeader;
	...
	private TradeLSCJListAdapter mListAdapter;
			...
			case TradeNetConnect.MSG_UPDATE_DATA: //
			{
				...
				if (nFrameType == 1)// 交易连接，交换密钥
				{
					...
					if (aMsgObject.nErrorCode < 0) {
						...
						new com.pengbo.mhdcx.widget.AlertDialog(TradeLiShiCJActivity.this).builder().setTitle("登录")
				...
				else if (nFrameType == Trade_Define.Func_LSCJ)//历史成交
				{
					closeProgress();
					if (aMsgObject.nErrorCode < 0) {
	...
	private void initView() {
		...
		mlLayout_lscjHeader = (LinearLayout) findViewById(R.id.lLayout_lscjheader);
		
		int iScreenWidth = ViewTools.getScreenSize(this.mContext).widthPixels;
		ViewGroup.LayoutParams params = mlLayout_lscjHeader.getLayoutParams();

		params.width = (iScreenWidth/4) * 12;
		mlLayout_lscjHeader.setLayoutParams(params);
		
		LinearLayout lLayout_cjHeader = (LinearLayout) findViewById(R.id.lLayout_lscxheader);
		lLayout_cjHeader.setVisibility(View.GONE);
		
		mListView_LSCX = (HVListView) findViewById(R.id.lv_lscx);
		mListView_LSCX.mListHead = mlLayout_lscjHeader;
		mListView_LSCX.setWidth(params.width);
		mListView_LSCX.setItemId(R.id.hv_lscj_item);
		...
		mListAdapter = new TradeLSCJListAdapter(mContext, mListData);
	...
	private void loadData()
	{
		...
		mMyApp.mTradeData.GetLSCJ(mListData);
    ...
    {    	
		...
		.setPositiveButton("确定", new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(TradeLiShiCJActivity.this, MainTabActivity.class);
				startActivity(intent);
				TradeLiShiCJActivity.this.finish();
			}
		}).show();
    }
