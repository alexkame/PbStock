...
public class TradeOptionListAll extends Fragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mView =  inflater.inflate(R.layout.trade_option_list_fragment, null);
	...
	private void initListView() {
		mHead = (View) mView.findViewById(R.id.headOfOptionList);
		mHead.setFocusable(true);
		mHead.setClickable(true);
		LayoutParams lp = null;
		int iScreenWidth = ViewTools.getScreenSize(context).widthPixels;
		for (int i = 0; i < AppConstants.LIST_HEADER_ITEMS; i++)
		{
			String strid = String.format("item%d", i+1);
			View tmp = mHead.findViewById(context.getResources().getIdentifier(strid, "id", context.getPackageName()));
		...
		if(mListView1 == null)
		{
			...
			mListAdapter = new CustomOptionListAdapter(context, mDatas, mHead);
