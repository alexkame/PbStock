 ...
 *
 */
public class BankToZQFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mView=inflater.inflate(R.layout.bank_rb1, null);
		initView();
		mIsViewReady = true;
		updateBankView();
		return mView ;
	...
	public void onAttach(Activity activity) {
		...
		try {
			mListener = (OnBankFragmentListener) activity;
	...
	public void onClick(View v) {
		...
		case R.id.llayout_banktozq_choosebank:
		{
			...
			intent.setClass(mActivity, TradeMoreChooseBankActivity.class);

			intent.putExtra("mmlx", mBankArrayName);
			intent.putExtra("pwtype", mSelectBankIndex);

			getActivity().startActivityForResult(intent, BankSecuritiesTransferActivity.REQUEST_CODE_BANK);
		}
			break;
	...
	public void updateCurrentBankIndex(int index)
	{
		...
		if (mBankArrayName != null && mSelectBankIndex < mBankArrayName.size())
		{
		    ...
		    nPwdFlag = STD.StringToInt(temp.get(BankSecuritiesTransferActivity.KEY_PWDFLAG));
